...
def create_user_directory(user_name, repo):
    ...
    for file_path in file_paths:
        ...
        response = requests.get(file_url, headers={"Authorization": f"token {os.environ['GH_TOKEN']}"})
        if response.status_code == 404:
            ...
            requests.put(file_url, json=payload, headers={"Authorization": f"token {os.environ['GH_TOKEN']}"})
...
def update_csv(user_name, repo, pr_name, draft_date, pr_number, repo_name):
    ...
    # Check if CSV exists
    response = requests.get(csv_url, headers={"Authorization": f"token {os.environ['GH_TOKEN']}"})
    ...
    requests.put(csv_url, json=payload, headers={"Authorization": f"token {os.environ['GH_TOKEN']}"})
...
def main():
    ...
    pr_response = requests.get(pr_url, headers={"Authorization": f"token {os.environ['GH_TOKEN']}"})
