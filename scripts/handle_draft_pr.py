...
def create_user_directory(user_name, repo):
    ...
    file_paths = [f"{base_path}/README.md", f"{base_path}/notable-contributions.md"]

    for file_path in file_paths:
        # Check if file exists, if not, create with basic content
        file_url = f"https://api.github.com/repos/{repo}/contents/{file_path}"
        response = requests.get(file_url, headers={"Authorization": f"token {os.environ['GITHUB_TOKEN']}"})
        if response.status_code == 404:
            # Create file
            content = "Initial content" if "README" in file_path else "List of notable contributions"
            payload = {
                "message": f"Create {file_path}",
                "content": content.encode("utf-8").decode("utf-8")
            }
            requests.put(file_url, json=payload, headers={"Authorization": f"token {os.environ['GITHUB_TOKEN']}"})
    
    return True

def update_csv(user_name, repo, pr_name, draft_date):
    ...
    # Append new data
    csv_data.append([draft_date, pr_name, "", "", "", "", "", ""])
...
def main():
    ...
    # Handle user directory and CSV
    if create_user_directory(user_name, repo):
        update_csv(user_name, repo, pr_name, draft_date)
